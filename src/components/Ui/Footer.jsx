...
const Footer = () => {
  return (
    <div className='dark:bg-[#202020] dark:text-white bg-[#F8F9FA] py-10 px-4 sm:px-10'>
      <div className='max-w-[1100px] mx-auto flex flex-col gap-8'>
          ...
          {contactInfo.map((item) => (
            <div key={item.id} className='flex items-center gap-4'>
              ...
              <div>
                ...
                <p className='text-sm dark:text-gray-300'>{item.detail}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-10 sm:gap-10'>
          <ul className='flex flex-col sm:flex-row gap-4 sm:gap-8'>
            ...
            <li className='hover:text-[#0D6EFD] hover:underline'>
